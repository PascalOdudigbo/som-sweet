...
function TextArea({label, inputValue, required, rows, cols, onChangeFunction}: Props) {
  return (
    <div className='textarea_wrapper'>
        ...
        <textarea className='textarea' rows={rows} cols={cols} required={required} value={inputValue.substring(0, 119)} onChange={onChangeFunction}/>
        <p className='input_count'>{inputValue.length}/120</p>
    </div>
  )
}
